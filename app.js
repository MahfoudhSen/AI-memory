...
class ChatLogVault {
    ...
    parseChatContent(content) {
        ...
        for (const line of lines) {
            if (line.toLowerCase().includes('user:') || line.toLowerCase().includes('you:')) {
                ...
                currentMessage = {
                    role: 'user',
                    text: line.replace(/^(user|you):\s*/i, '').trim()
                };
                ...
                currentMessage = {
                    role: 'ai',
                    text: line.replace(/^(assistant|chatgpt|ai):\s*/i, '').trim()
                };
    ...
    async saveChat() {
        ...
        if (messages.length === 0) {
            this.showToast('No valid messages found. Please check the format.', 'error');
            return;
        }
